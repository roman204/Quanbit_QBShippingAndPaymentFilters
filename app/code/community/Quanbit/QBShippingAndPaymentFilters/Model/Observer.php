...
class Quanbit_QBShippingAndPaymentFilters_Model_Observer
{
    ...
    public function addRemoteIP($observer)
    {
        /* @var $object Mage_Sales_Model_Quote */
        $object = $observer->getEvent()->getDataObject();
        $addressCollection = $object->getAddressesCollection();
        foreach($addressCollection as $addressModel) {
            $remote_ip = explode(",", $_SERVER['HTTP_X_FORWARDED_FOR']);
            $remote_ip = $remote_ip[0];
            $addressModel->setData('remote_ip', $remote_ip);
            //$addressModel->setData('remote_ip', $_SERVER['REMOTE_ADDR']);
        }
    }
